...
# unlimited.
# ----------------------------------------------------------------------------
"""General Instrument for loading locally-written netCDF files.

Properties
----------
platform
    'gdm', will be updated if file contains a platform attribute
name
...
::
    ...
    fname = "test_gdm_file_%Y%j.nc"
    inst.to_netcdf4(fname=inst.date.strftime(fname))

    # Load the NetCDF file
    file_inst = gdm.Instrument(
        "gdm", "netcdf", temporary_file_list=True, directory_format="./",
...
def download(date_array, tag, inst_id, data_path=None):
    ...
    data_path : str or NoneType
        Path to directory where the data will be download data. (default=None)

    """

    warnings.warn("".join(["Downloads are not currently supported for ",
                           "GeospaceDataManagement netCDF files"]))
    return
...
def load(fnames, tag='', inst_id='', strict_meta=False, file_format='NETCDF4',
    ...
    epoch_name : str or NoneType
        ...
        across GeospaceDataManagement instruments.  (default=None)
    epoch_unit : str
