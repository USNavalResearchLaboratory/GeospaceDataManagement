...
# ----------------------------------------------------------------------------
# -*- coding: utf-8 -*-
"""Provides generalized routines for integrating instruments."""
...
def is_daily_file_cadence(file_cadence):
    ...
    file_cadence : dt.timedelta or pds.DateOffset
        GeospaceDataManagement assumes a daily file cadence, but some instrument
        data file contain longer periods of time.  This parameter allows the
        specification of regular file cadences greater than or equal to a day
        (e.g., weekly, monthly, or yearly). (default=dt.timedelta(days=1))
...
def list_files(tag='', inst_id='', data_path='', format_str=None,
    ...
    tag : str
        ...
        This input is nominally provided by GeospaceDataMangement. (default='')
    inst_id : str
        Instrument ID used to identify particular data set to be loaded.
        This input is nominally provided by GeospaceDataMangement. (default='')
    data_path : str
        Path to data directory. This input is nominally provided by
        GeospaceDataMangement. (default='')
    ...
    file_cadence : dt.timedelta or pds.DateOffset
        GeospaceDataMangement assumes a daily file cadence, but some instrument
        data file contain longer periods of time.  This parameter allows the
        specification of regular file cadences greater than or equal to a day
        (e.g., weekly, monthly, or yearly). (default=dt.timedelta(days=1))
    ...
    Returns
    -------
    out : GeospaceDataManagement._files.Files
        A class containing the verified available files

    See Also
    --------
    GeospaceDataMangement.Files.from_os

    Notes
    -----
    This routine provides a standard interface for instrument modules, and
    is intended to be invoked by GeospaceDataManagement rather than the user.

    Examples
    --------
    ::

        from GeospaceDataManagement.instruments.methods import general as mm_gen
    ...
    if format_str is None:
        # gdm performs a check against `inst_id` and `tag` before calling
        # `list_files`. However, supported_tags is a non-gdm input.
        try:
    ...
    out = gdm.Files.from_os(data_path=data_path, format_str=format_str,
...
def remove_leading_text(inst, target=None):
    ...
    ----------
    inst : gdm.Instrument
        Associated gdm.Instrument object
...
def filename_creator(value, format_str=None, start_date=None, stop_date=None):
    """Create filenames as needed to support use of generated data sets.
    ...
    estr = ''.join(('This feature has not been implemented yet and is here ',
                    'to support experimentation by the gdm team. If you are ',
                    'here intentionally, please contact the gdm developers ',
                    'and let us know what you are trying to accomplish so we',
                    ' can evaluate supporting the desired use case.'))
