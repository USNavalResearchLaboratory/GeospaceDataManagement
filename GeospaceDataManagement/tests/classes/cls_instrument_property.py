...
# unlimited.
# ----------------------------------------------------------------------------
"""Test for instrument properties in the Instrument object and methods.
...
class InstPropertyTests(object):
    """Basic tests for `gdm.Instrument` properties.
    ...
    testing_kwargs : dict
        ...
        the range of `gdm.Instrument` dataset module methods
    xarray_epoch_name : str
        String used within a xarray testing `gdm.Instrument` object
    ...
    See Also
    --------
    GeospaceDataManagement.tests.test_instrument
    ...
    def test_download_updated_files(self, caplog, no_remote_files,
        ...
        if no_remote_files:
            ...
            if len(download_keys) == 0:
                ...
                assert "Updating gdm file list" not in caplog.text
            else:
                assert "Downloading data to" in caplog.text
                assert "Updating gdm file list" in caplog.text
        else:
            assert "Downloading data to" in caplog.text
            assert "Updating gdm file list" in caplog.text
    ...
    def test_basic_str(self):
        ...
        assert isinstance(self.out, str)
        assert self.out.find('GeospaceDataManagement Instrument object') == 0
    ...
    def test_instrument_function_keywords(self, caplog, func, kwarg, val):
        """Test if Instrument function keywords are registered.
    ...
    def test_instrument_function_keyword_liveness(self, caplog, func, kwarg):
        """Test if changed keywords are propagated to functions.
