...
# unlimited.
# ----------------------------------------------------------------------------
"""Tests the GeospaceDataManagement utils core functions."""
...
class TestCIonly(object):
    ...
    def test_initial_gdm_load(self, capsys):
        ...
        root = os.path.join(os.path.expanduser("~"), '.gdm')
        new_root = os.path.join(os.path.expanduser("~"), '.saved_gdm')
        ...
        assert os.path.isfile(os.path.join(root, 'gdm_settings.json'))
        ...
        # Make sure GeospaceDataManagement reloads settings
        reload(gdm)
        return
...
class TestIfyFunctions(object):
    ...
    def test_stringify(self, strlike):
        ...
        output = gdm.utils.stringify(strlike)
    ...
    def test_stringify_non_str_types(self, astrlike):
        ...
        output = gdm.utils.stringify(astrlike)
...
class TestAvailableInst(TestWithRegistration):
    ...
    @pytest.mark.parametrize("inst_loc", [None, gdm.instruments])
    ...
    def test_display_available_instruments(self, inst_loc, inst_flag,
        ...
        # If using the GeospaceDataManagement registry, make sure there is
    ...
    def test_display_instrument_stats(self, inst_loc, capsys):
        ...
        captured = capsys.readouterr()
        # Numbers should match supported data products in `gdm.instruments`
...
class TestGenerateInstList(object):
    ...
    def setup_method(self):
        ...
        self.user_info = {'gdm_testmodel': {'user': 'GideonNav',
                                            'password': 'pasSWORD!'}}
        self.inst_list = utils.generate_instrument_list(
            inst_loc=gdm.instruments, user_info=self.user_info)
        return
    ...
    def test_user_info_pass_through(self):
        ...
        for inst in self.inst_list['download']:
            # `user_info` should only be in `gdm_testmodel`
            assert (('user_info' in inst.keys())
                    == ('gdm_testmodel' in str(inst['inst_module'])))
            if 'user_info' in inst.keys():
                # User info should be correct
                assert inst['user_info'] == self.user_info['gdm_testmodel']
