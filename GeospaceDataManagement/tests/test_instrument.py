...
# ----------------------------------------------------------------------------
# -*- coding: utf-8 -*-
"""Tests the Instrument object and methods."""
...
import GeospaceDataManagement.instruments.gdm_ndtesting
import GeospaceDataManagement.instruments.gdm_testing

from GeospaceDataManagement.tests.classes.cls_instrument_access import \
    InstAccessTests
from GeospaceDataManagement.tests.classes.cls_instrument_integration import \
    InstIntegrationTests
from GeospaceDataManagement.tests.classes.cls_instrument_iteration import \
    InstIterationTests
from GeospaceDataManagement.tests.classes.cls_instrument_property import \
    InstPropertyTests
...
class TestBasics(InstAccessTests, InstIntegrationTests, InstIterationTests,
                 InstPropertyTests):
    """Unit tests for gdm.Instrument object."""
    ...
    def setup_method(self):
        ...
        self.testInst = gdm.Instrument(platform='gdm', name='testing',
                                       ...
                                       update_files=True, **self.testing_kwargs)
...
class TestInstCadence(TestBasics):
    """Unit tests for gdm.Instrument objects with the default file cadance."""
...
class TestInstMonthlyCadence(TestInstCadence):
    """Unit tests for gdm.Instrument objects with a monthly file cadance."""
...
class TestBasicsInstModule(TestBasics):
    ...
    def setup_method(self):
        ...
        reload(gdm.instruments.gdm_testing)
        imod = gdm.instruments.gdm_testing
        self.testInst = gdm.Instrument(inst_module=imod, num_samples=10,
...
class TestBasicsNDXarray(TestBasics):
    """Basic tests for ND xarray `gdm.Instrument`.
    ...
    def setup_method(self):
        ...
        reload(gdm.instruments.gdm_ndtesting)
        self.testInst = gdm.Instrument(platform='gdm', name='ndtesting',
                                       num_samples=10, clean_level='clean',
                                       update_files=True,
                                       **self.testing_kwargs)
        self.ref_time = gdm.instruments.gdm_ndtesting._test_dates['']['']
        self.ref_doy = int(self.ref_time.strftime('%j'))
        self.out = None
        return
...
class TestBasicsShiftedFileDates(TestBasics):
    """Basic tests for pandas `gdm.Instrument` with shifted file dates."""
...
class TestInstGeneral(object):
    ...
    def setup_method(self):
        ...
        self.empty_inst = gdm.Instrument()
        return
    ...
    def test_creating_empty_instrument_object(self):
        ...
        assert isinstance(self.empty_inst, gdm.Instrument)
        return

    def test_load_empty_instrument_no_files_error(self):
        """Ensure error loading Instrument with no files."""

        # Trying to load when there are no files a UserWarning
        with warnings.catch_warnings(record=True) as self.war:
            self.empty_inst.load()
        testing.eval_warnings(self.war,
                              'No files found for Instrument',
    ...
    def test_empty_repr_eval(self):
        ...
        assert isinstance(self.out, gdm.Instrument)
    ...
    def test_supplying_instrument_module_requires_name_and_platform(self):
        ...
        testing.eval_bad_input(gdm.Instrument, AttributeError,
                               'Supplied module ',
                               input_kwargs={'inst_module': Dummy})
        return

    def test_eq_different_object(self):
        """Test equality using different `gdm.Instrument` objects."""

        obj1 = gdm.Instrument(platform='gdm', name='testing',
                              num_samples=10, clean_level='clean',
                              update_files=True)

        obj2 = gdm.Instrument(platform='gdm', name='ndtesting',
                              num_samples=10, clean_level='clean',
                              update_files=True)
...
class TestDeprecation(object):
    ...
    def setup_method(self):
        ...
        self.in_kwargs = {"platform": 'gdm', "name": 'testing',
                          "clean_level": 'clean'}
        self.ref_time = gdm.instruments.gdm_testing._test_dates['']['']
        self.warn_msgs = []
        self.war = ""
