...
from GeospaceDataManagement.instruments.methods import general as gen
...
class TestListFiles(object):
    """Unit tests for `gdm.instrument.methods.general.list_files`."""
...
class TestRemoveLeadText(object):
    """Unit tests for `gdm.instrument.methods.general.remove_leading_text`."""

    def setup_method(self):
        """Set up the unit test environment for each method."""

        # Load a test instrument
        self.testInst = gdm.Instrument('gdm', 'testing', num_samples=12,
                                       clean_level='clean')
...
class TestRemoveLeadTextXarray(TestRemoveLeadText):
    ...
    def setup_method(self):
        ...
        # Load a test instrument
        self.testInst = gdm.Instrument('gdm', 'ndtesting', num_samples=12,
                                       clean_level='clean')
        self.testInst.load(2009, 1)
        self.npts = len(self.testInst['uts'])
        return
...
class TestLoadCSVData(object):
    """Unit tests for `gdm.instrument.methods.general.load_csv_data`."""
