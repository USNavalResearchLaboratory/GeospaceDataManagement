...
# unlimited.
# ----------------------------------------------------------------------------
"""Test GeospaceDataManagement Files object and code."""
...
def create_dir(inst=None, temporary_file_list=False):
    ...
    inst : gdm.Instrument or NoneType
        Instrument to create directory for. If `None`, then a 'gdm'
        'testing' Instrument created. (default=None)
    temporary_file_list : bool
        Flag passed to `gdm.Instrument` if `inst` is None (default=False)

    """
    if inst is None:
        # Create instrument
        inst = gdm.Instrument(platform='gdm', name='testing',
...
def list_files(tag=None, inst_id=None, data_path=None, format_str=None,
    ...
    if format_str is None:
        if version:
            format_str = ''.join(('gdm_testing_junk_{year:04d}_{month:02d}_',
                                  '{day:03d}{hour:02d}{minute:02d}{second:02d}',
                                  '_stuff_{version:02d}_{revision:03d}_',
                                  '{cycle:02d}.gdm_testing_file'))
        else:
            format_str = ''.join(('gdm_testing_junk_{year:04d}_gold_',
    ...
    if tag is not None:
        if tag == '':
            return gdm.Files.from_os(data_path=data_path, format_str=format_str)
        else:
...
class TestNoDataDir(object):
    ...
    def setup_method(self):
        ...
        # Store the current gdm directory
        self.saved_data_path = gdm.params['data_dirs']

        gdm.params.data['data_dirs'] = []
        reload(gdm._files)
        return

    def teardown_method(self):
        """Clean up the unit test environment after each method."""
        gdm.params.data['data_dirs'] = self.saved_data_path
        reload(gdm._files)
        return

    def test_no_data_dir(self):
        """Test that error is raised if no data directory is specified."""
        testing.eval_bad_input(gdm.Instrument, NameError,
                               "Please set a top-level directory path")
        return


class TestBasics(object):
    """Unit tests for `gdm._files`."""
    ...
    def setup_method(self):
        ...
        # Store current gdm directory
        self.data_paths = gdm.params['data_dirs']
        ...
        self.testInst = gdm.Instrument(
            inst_module=gdm.instruments.gdm_testing, clean_level='clean',
    ...
    def teardown_method(self):
        """Clean up the unit test environment after each method."""
        gdm.params['data_dirs'] = self.data_paths
    ...
    @pytest.mark.parametrize("root_fname,freq,use_doy",
                             [[''.join(['gdm_testing_junk_{year:04d}_gold_',
                                       '{day:03d}_stuff.gdm_testing_file']),
                               '1D', True],
                              [''.join(['{year:04d}_gold_gdm_testing_junk_',
                                        '{day:03d}_stuff.gdm_testing_file']),
                               '1D', True],
                              [''.join(['{year:04d}{day:03d}.gdm_testing_',
                                        'file']),
                               '1D', True],
                              [''.join(['gdm_testing_junk_{year:04d}',
                                        '{day:03d}_stuff.gdm_testing_file']),
                               '1D', True],
                              [''.join(['gdm_testing_junk_{year:04d}_gold_',
                                        '{day:03d}_stuff_{month:02d}.gdm_',
                                        'testing_file']),
                               '1D', False],
                              [''.join(['gdm_testing_junk_{year:04d}_gold_',
                                        '{day:03d}_stuff_{month:02d}_',
                                        '{hour:02d}.gdm_testing_file']),
                               '6h', False],
                              [''.join(['gdm_testing_fromos_{year:04d}_gold_',
                                        '{day:03d}_stuff_{month:02d}_',
                                        '{hour:02d}{minute:02d}.',
                                        'gdm_testing_file']),
                               '30min', False],
                              [''.join(['gdm_testing_hms_{year:04d}_gold_',
                                        '{day:03d}_stuff_{month:02d}_',
                                        '{hour:02d}_{minute:02d}_{second:02d}',
                                        '.gdm_testing_file']),
                               '30s', False],
                              [''.join(['gdm_testing_junk_{year:04d}_gold_',
                                        'stuff_{month:02d}.gdm_testing_file'
                                        ]),
                               '1MS', False]])
    ...
    def test_from_os(self, delimiter, root_fname, freq, use_doy):
        ...
        # Truth dates
        dates = gdm.utils.time.create_date_range(self.start, self.stop, freq)
        if len(dates) > 100000:
            self.stop = self.start + dt.timedelta(days=1)
            dates = gdm.utils.time.create_date_range(self.start, self.stop,
                                                     freq)
    ...
    @pytest.mark.parametrize(
        ...
        [['gdm_1234567_junk_{year:04d}_gold_{day:03d}_stuff',
          'gdm_{code:7s}_junk_{year:04d}_gold_{day:03d}_stuff'],
         ['gdm_1234567_junk_{year:04d}_gold_{day:03d}_stuff',
    ...
    def test_from_os_user_vars(self, delimiter, root_fname, root_pname):
        ...
        # Add suffix to denote gdm testing file
        root_fname = '.'.join((root_fname, 'gdm_testing_file'))
        root_pname = '.'.join((root_pname, 'gdm_testing_file'))

        # Truth dates
        dates = gdm.utils.time.create_date_range(self.start, self.stop, '1D')
    ...
    @pytest.mark.parametrize("root_fname,root_pname",
                             [[''.join(['gdm_1234567_junk_{year:04d}_gold_',
                                       '{day:03d}_stuff.gdm_testing_file']),
                               ''.join(['gdm_*_junk_{year:04d}_gold_',
                                        '{day:03d}_stuff.gdm_testing_file'])],
                              [''.join(['gdm_1234567_junk_{year:04d}_gold_',
                                        '{day:03d}_stuff.gdm_testing_file']),
                               ''.join(['gdm_123*_junk_{year:04d}_',
                                        'gold_{day:03d}_stuff.gdm_testing',
                                        '_file'])],
                              [''.join(['gdm_1234567_junk_{year:04d}_gold_',
                                        '{day:03d}_stuff.gdm_testing_file']),
    ...
    @pytest.mark.parametrize("root_fname,root_pname,user_vars,truth_vals",
                             [[''.join(['gdm_1234567_junk_{year:04d}_gold_',
                                       '{day:03d}_stuff.gdm-testing-file']),
                               ''.join(['gdm_{code:7d}_junk_{year:04d}',
                                        '_{d:4}_{day:03d}_stuff.gdm-testing-',
                                        'file']),
                               ['code', 'd'], [1234567, 'gold']],
                              [''.join(['gdm_1234567_junk_{year:04d}_gold_',
    ...
    def test_default_directory_format(self):
        ...
        files = gdm.Files(self.testInst)
        assert files.directory_format == gdm.params['directory_format']
        return
...
class TestInstWithFiles(object):
    ...
    def setup_method(self):
        ...
        # create a bunch of files by year and doy
        self.testInst = gdm.Instrument(
            inst_module=gdm.instruments.gdm_testing,
            clean_level='clean',
            temporary_file_list=self.temporary_file_list,
            update_files=True)

        self.root_fname = ''.join(('gdm_testing_junk_{year:04d}_gold_',
                                   '{day:03d}_stuff_{month:02d}_{hour:02d}_',
                                   '{minute:02d}_{second:02d}.gdm_testing_',
                                   'file'))
    ...
    def teardown_method(self):
        ...
        gdm.Instrument(inst_module=gdm.instruments.gdm_testing,
    ...
    def test_instrument_with_ignore_empty_files(self):
        ...
        self.testInst = gdm.Instrument(
            ...
            clean_level='clean', update_files=True,
            temporary_file_list=self.temporary_file_list,
            file_format=self.root_fname, ignore_empty_files=True)
    ...
    def test_get_new_files_after_adding_files(self):
        ...
        create_files(self.testInst, self.start2, self.stop2, freq='100min',
                     use_doy=False, root_fname=self.root_fname,
                     version=self.version)
        dates = gdm.utils.time.create_date_range(self.start2, self.stop2,
                                                 freq='100min')
    ...
    def test_get_new_files_after_adding_files_and_adding_file(self):
        ...
        create_files(self.testInst, start, stop, freq='100min',
                     use_doy=False, root_fname=self.root_fname,
                     version=self.version)
        dates2 = gdm.utils.time.create_date_range(start, stop, freq='100min')
    ...
    def test_get_new_files_after_deleting_files_and_adding_files(self):
        ...
        for the_file in os.listdir(self.testInst.files.data_path):
            if (the_file[0:13] == 'gdm_testing') and (
                    the_file[-19:] == '.gdm_testing_file'):
...
class TestInstWithFilesNonStandard(object):
    ...
    def test_files_non_standard_gdm_directory(self):
        """Check that files work with a weird directory structure."""
        # Create new files and make sure that new files are captured
        dates = gdm.utils.time.create_date_range(self.start, self.stop,
                                                 freq='100min')

        nonstandard_dir = 'gdm_testing_{tag}_{inst_id}'

        self.testInst = gdm.Instrument(
            inst_module=gdm.instruments.gdm_testing,
            clean_level='clean', directory_format=nonstandard_dir,
    ...
    def test_files_non_standard_file_format_template(self):
        ...
        dates = gdm.utils.time.create_date_range(self.start, self.stop,
                                                 freq='1D')
        if self.version:
            root_fname = ''.join(('gdm_testing_unique_{version:02d}_',
                                  '{revision:03d}_{cycle:02d}_{year:04d}',
                                  '_g_{day:03d}_st.gdm_testing_file'))
        else:
            root_fname = ''.join(('gdm_testing_unique_junk_{year:04d}_gold',
                                  '_{day:03d}_stuff.gdm_testing_file'))

        self.testInst = gdm.Instrument(
            inst_module=gdm.instruments.gdm_testing, clean_level='clean',
            file_format=root_fname, update_files=True,
            temporary_file_list=self.temporary_file_list)
    ...
    @pytest.mark.parametrize('file_format', [
        'gdm_testing_unique_junk_stuff.gdm_testing_file', 15])
    def test_files_non_standard_file_format_template_fail(self, file_format):
        ...
        in_kwargs = {'inst_module': gdm.instruments.gdm_testing,
                     'clean_level': 'clean', 'file_format': file_format,
                     'update_files': True,
                     'temporary_file_list': self.temporary_file_list}

        testing.eval_bad_input(gdm.Instrument, ValueError,
                               'Supplied format string', input_kwargs=in_kwargs)
        return
...
class TestInstWithVersionedFilesNonStandard(TestInstWithFilesNonStandard):
    ...
    def test_files_when_duplicates_forced(self):
        ...
        file_format = ''.join(('gdm_testing_unique_{version:02d}_',
                               '{revision:03d}_{cycle:02d}_{year:04d}',
                               '_g_{day:03d}_st.gdm_testing_file'))
        self.testInst = gdm.Instrument(
            inst_module=gdm.instruments.gdm_testing,
        ...
        self.testInst = gdm.Instrument(
            inst_module=gdm.instruments.gdm_testing,
            clean_level='clean', file_format=file_format, update_files=True,
            temporary_file_list=self.temporary_file_list)
        assert (np.all(self.testInst.files.files.index == dates))
        return
...
def create_instrument(j):
    ...
    `update_files` should update the file list in .gdm
...
class TestFilesRaceCondition(object):
    """Tests for multiple instances of gdm running simultaneously."""
    ...
    def setup_method(self):
        ...
        self.root_fname = ''.join(('gdm_testing_junk_{year:04d}_{month:02d}',
        ...
        self.testInst = gdm.Instrument(
            inst_module=gdm.instruments.gdm_testing, clean_level='clean',
            update_files=True, temporary_file_list=self.temporary_file_list)
    ...
    def test_race_condition(self):
        """Test that multiple instances of gdm instrument creation run."""
        processes = 50
...
class TestCIonly(CICleanSetup):
    ...
    def test_initial_gdm_load(self, capsys):
        """Ensure data_dirs check in Files works."""

        # Ensure gdm is running in 'first-time' mode
        reload(gdm)
        captured = capsys.readouterr()
        assert captured.out.find("Hi there!") >= 0

        # Ensure the gdm 'data_dirs' param is empty list on both
        # local systems and CI.
        gdm.params.data['data_dirs'] = []

        # Evaluate Instrument instantiation failure
        testing.eval_bad_input(gdm.Instrument, NameError,
                               "".join(["GeospaceDataManagement's `data_dirs` ",
                                        "hasn't been set."]),
                               input_args=['gdm', 'testing'])
        return
