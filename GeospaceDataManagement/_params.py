...
class Parameters(object):
    """Stores user parameters used by GeospaceDataManagement.

    Also stores custom user parameters provided the keys don't conflict with
    default GeospaceDataManagement parameters.

    Parameters
    ----------
    path : str
        If provided, the directory path will be used to load/store a
        parameters file with name 'gdm_settings.json' (default=None)
    create_new : bool
        If True, a new parameters file is created. Will be created at path
        if provided. If not, file will be created in `.gdm` directory
        stored under the user's home directory.

    Attributes
    ----------
    data : dict
        GeospaceDataManagement user settings dictionary
    defaults : dict
        Default parameters (keys) and values used by GeospaceDataManagement that
        include:
    ...
    non_defaults : list
        List of GeospaceDataManagement parameters (strings) that don't have a
        defined default and are unaffected by `self.restore_defaults()`

    Raises
    ------
    ValueError
        The 'user_modules' parameter may not be set directly by the user.
        Please use the `GeospaceDataManagement.utils.regsitry` module to modify
    ...
    All GeospaceDataManagement parameters are automatically stored whenever a
    parameter is assigned or modified. The default parameters and values tracked
    by this class are grouped by type below.

    Values that map to the corresponding keywords on
    GeospaceDataManagement.Instrument: clean_level, directory_format,
    ignore_empty_files, and update_files.  See the Instrument docstring for
    more information on these keywords.

    Values that map to internal GeospaceDataManagement settings: file_timeout,
    user_modules, and warn_empty_file_list.

    Stored GeospaceDataManagement parameters without a working default value:
    data_dirs.

    file_timeout -  Time in seconds that GeospaceDataManagement will wait to
    modify a busy file

    user_modules - Stores information on modules registered by
    ...
    def __init__(self, path=None, create_new=False):
        ...
        # Define stored GeospaceDataManagement parameters without a default
        # setting
        ...
        if path is not None:
            ...
            # Cycle through locations and load any GeospaceDataManagement
            # parameter files found. First, check current working directory,
            # then GeospaceDataManagement user directory. Stop looking once an
            #  existing file is found.
            file_locs = [os.path.join('.', sfname),
                         os.path.join(os.path.expanduser('~'), '.gdm',
                                      sfname)]
        ...
        # Can't use NetworkLock in gdm.utils either since this object (params)
        # doesn't exist yet.
    ...
    def _set_data_dirs(self, path=None, store=True):
        """Set the top level directories gdm uses to store and load data.
    ...
    def clear_and_restart(self):
        ...
        # Set parameters without a default working value to []
    ...
    def restore_defaults(self):
        ...
        # Set default values for each of the gdm provided values. Set all but
        # last parameter directly. Set last using __setitem__ to trigger a file
        # write.
