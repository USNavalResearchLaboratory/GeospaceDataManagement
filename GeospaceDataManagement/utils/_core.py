...
def get_mapped_value(value, mapper):
    ...
    mapped_val : str or NoneType
        Adjusted variable name or NoneType if input value should stay the same
        ...
        try:
            ...
            for iid in info.keys():
                for tag in info[iid].keys():
                    ...
                    # Initialize instrument so that GeospaceDataManagement can
                    # generate skip flags where appropriate
                    inst = gdm.Instrument(inst_module=module, tag=tag,
                                          inst_id=iid, temporary_file_list=True)
                        ...
                        if inst._test_download:
                            ...
                            if hasattr(module, '_test_load_opt'):
                                ...
                                try:
                                    ...
                                    kw_list = gdm.utils.listify(kw_list)
...
def display_instrument_stats(inst_locs=None):
    ...
    inst_locs : list of packages
        List of instrument library modules to inspect for GeospaceDataManagement
        support. If None, report on default GeospaceDataManagement package.
        (default=None)

    """

    if inst_locs is None:
        inst_locs = [gdm.instruments]
...
def available_instruments(inst_loc=None):
    ...
    inst_loc : python subpackage or NoneType
        The location of the instrument subpackage (e.g., gdm.instruments)
...
def update_fill_values(inst, variables=None, new_fill_val=np.nan):
    ...
    """
    raise RuntimeError('hi, need to fix this for new meta structure')
    ...
    def __init__(self, *args, **kwargs):
        ...
        **kwargs : dict reference
        ...
        if 'timeout' in kwargs:
            ...
            timeout = gdm.params['file_timeout']
