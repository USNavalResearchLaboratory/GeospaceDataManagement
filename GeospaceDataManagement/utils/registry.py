...
# unlimited.
# ----------------------------------------------------------------------------
"""GeospaceDataManagement user module registry utilities.

This module allows GeospaceDataManagement to provide direct access to external
or custom instrument modules by maintaining information about these instrument
modules.
...
gdm.params['user_modules'] and stored as a dict of dicts keyed by platform
and name.
...
::
    ...
    # Register published packages that depend on GeospaceDataManagement
    import gdmInstruments
    import gdmModels
    registry.register_by_module(gdmInstruments.instruments)
    registry.register_by_module(gdmInstruments.models)
...
def load_saved_modules():
    """Load registered gdm.Instrument modules.
    ...
    return gdm.params['user_modules']
...
def register(module_names, overwrite=False):
    """Register a user gdm.Instrument module by name.

    Enables instantiation of a third-party Instrument
    module using
    ::

        inst = gdm.Instrument(platform, name, tag=tag, inst_id=inst_id)
    ...
    Warnings
    --------
    Registering a module that contains code other than GeospaceDataManagement
    ...
    Module names do not have to follow the GeospaceDataManagement platform_name
    naming convection.
    ...
    ::
        ...
        from GeospaceDataManagement import Instrument
        from gdm.utils import registry
    ...
    for mod_name in module_names:
        ...
        # Second, check that module is itself GeospaceDataManagement compatible
        ...
        if platform not in gdm.params['user_modules']:
            # setup `of dict` part of dict of dicts
            gdm.params.data['user_modules'][platform] = {}

        # Only register name if it is not present under platform
        if name not in gdm.params['user_modules'][platform]:
            gdm.logger.info('Registering user module {}'.format(mod_name))
            # Add to current user modules structure and store it to disk
            gdm.params.data['user_modules'][platform][name] = mod_name
            store()
        else:
            # Platform/name combination already registered. Check to see if
            # this is a new package or just a redundant assignment
            if mod_name != gdm.params['user_modules'][platform][name]:
...
def register_by_module(module, overwrite=False):
    ...
    module : Python module
        Module with one or more gdm.Instrument support modules
    ...
    ::
        ...
        inst = gdm.Instrument(platform, name)
    ...
    ::
        ...
        import gdm_fun_package
        gdm.utils.registry.register_by_module(gdm_fun_package.models)
...
def remove(platforms, names):
    ...
    Notes
    -----
    Current registered user modules available at gdm.params['user_modules']
    ...
    for platform, name in zip(platforms, names):
        if platform in gdm.params['user_modules']:
            if name is None:
                # remove platform entirely
                gdm.params['user_modules'].pop(platform)
                # store
                store()
            else:
                # name supplied, remove single module
                if name in gdm.params['user_modules'][platform]:
                    # remove module
                    gdm.params['user_modules'][platform].pop(name)
                else:
                    # name not in platform
                    estr = ''.join((platform, ', ', name, ': not a registered ',
                                    'instrument module.'))
                    gdm.logger.info(estr)
                # remove platform if no remaining instruments
                if len(gdm.params['user_modules'][platform]) == 0:
                    gdm.params['user_modules'].pop(platform)
                # store
                store()
        else:
