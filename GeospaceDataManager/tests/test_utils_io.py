...
class TestNetCDF4Integration(object):
    ...
    def test_meta_translation_to_from_netcdf4(self, assign_flag, meta_trans):
        ...
        # Write the file
        gdm.utils.files.check_and_make_path(self.tempdir.name)
        outfile = os.path.join(self.tempdir.name,
                               'gdm_test_ncdf.nc')
        ...
        gdm.utils.io.inst_to_netcdf(self.testInst, outfile,
                                    epoch_name=default_epoch_name, **mkwargs)
        ...
        # Load file using gdm utilities and test translation of multiple
    ...
    def test_meta_processor_to_from_netcdf4(self, assign_flag):
        ...
        # Target meta info
        target = {'testing_metadata_gdm_answer': '42',
                  'testing_metadata_gdm_question': 'simulation running'}
...
class TestXarrayIO(object):
    ...
    def test_gdm_meta_to_xarray_attr(self):
        ...
        io.gdm_meta_to_xarray_attr(self.testInst.data, meta, self.epoch_name)
...
class TestMetaTranslation(object):
    ...
    def setup_method(self):
        ...
        self.test_inst = gdm.Instrument('gdm', 'testing', num_samples=5)
        self.test_date = gdm.instruments.gdm_testing._test_dates['']['']
    ...
    def test_from_file_table_translation_missing(self, caplog):
        ...
        # Apply inverse
        with caplog.at_level(logging.DEBUG, logger='GeospaceDataManagement'):
            io.apply_table_translation_from_file(from_trans, self.out)
    ...
    def test_remove_netcdf4_standards(self, caplog):
        ...
        # When loading files, GeospaceDataManagement specifically checks for
        # 'Epoch' as the long_name. So, ensuring long_name for such variables
        # is written could break loading for existent files. I could fake it,
        # and assign the standard name as long_name when loading, and while that
        # would pass the tests here as written, it would be brittle. Check
        # everything else.
...
class TestMetaTranslationXarray(TestMetaTranslation):
    ...
    def setup_method(self):
        ...
        self.test_inst = gdm.Instrument('gdm', 'ndtesting', num_samples=5)
        self.test_date = gdm.instruments.gdm_ndtesting._test_dates['']['']
        self.test_inst.load(date=self.test_date)
        self.meta_dict = self.test_inst.meta.to_dict()
        self.out = None
...
class TestMetaTranslationModel(TestMetaTranslation):
    ...
    def setup_method(self):
        ...
        self.test_inst = gdm.Instrument('gdm', 'testmodel', num_samples=5)
        self.test_date = gdm.instruments.gdm_testmodel._test_dates['']['']
        self.test_inst.load(date=self.test_date)
        self.meta_dict = self.test_inst.meta.to_dict()
        self.out = None
