...
def ensure_updated_stored_modules(modules):
    """Ensure stored gdm.params['user_modules'] updated to include modules.
...
def ensure_live_registry_updated(modules):
    """Ensure current gdm.params['user_modules'] updated to include modules.
...
class TestRegistration(TestWithRegistration):
    ...
    def test_duplicate_registration_error(self):
        ...
        sys.modules['gdm_error.test_faux_module'] = sys.modules[mod_name]
        ...
        testing.eval_bad_input(
            ...
            input_args=[['gdm_error.test_faux_module']])
    ...
    def test_duplicate_registration_overwrite(self):
        ...
        registry.register(['gdm_error.test_faux_module'], overwrite=True)
        self.modules[0] = ('gdm_error.test_faux_module', 'platname1', 'name1')
    ...
    def test_platform_removal_not_present(self):
        ...
        stored_modules = copy.deepcopy(gdm.params['user_modules'])
        registry.remove(['made_up_name'], [None])

        # Make sure nothing changed
        assert stored_modules == gdm.params['user_modules']
