...
# unlimited.
# ----------------------------------------------------------------------------
"""Standardized class and functions to test registration for libraries.
...
class TestWithRegistration(object):
    ...
    def setup_method(self):
        ...
        gdm.utils.registry.remove(self.platforms, self.names)
    ...
    def ensure_not_in_stored_modules(self):
        """Ensure modules not in stored gdm.user_modules.
        ...
        saved_modules = gdm.utils.registry.load_saved_modules()
        ...
        for self.module_name, self.platform, self.name in self.modules:
            ...
            testing.eval_bad_input(gdm.Instrument, KeyError, 'unknown',
    ...
    def create_fake_module(self):
        """Create fake module and package from gdm_testing test Instrument."""

        # Use gdm_testing as base instrument
        file_path = gdm.instruments.gdm_testing.__file__
    ...
    def verify_inst_instantiation(self):
        ...
        Notes
        -----
        Checks for presence in gdm.user_modules
        ...
        if self.module_name is None:
            ...
            inst = gdm.Instrument(self.platform, self.name)
        else:
            # Load Instrument by module
            inst = gdm.Instrument(inst_module=self.module_name)
