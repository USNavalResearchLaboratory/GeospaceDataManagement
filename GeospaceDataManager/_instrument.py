...
class Instrument(object):
    ...
    clean_level : str or NoneType
        ...
        setting in `gdm.params['clean_level']`. (default=None)
    update_files : bool or NoneType
        ...
        this setting will be obtained from `gdm.params` (default=None)
    ...
    data_dir : str
        ...
        bypass the directories provided by gdm.params['data_dirs'].  Only
    ...
    directory_format : str, function, or NoneType
        ...
        specified, is provided by gdm.params['directory_format'] and is
    ...
    file_format : str or NoneType
        ...
        `gdm.utils.files.parse_delimited_filenames` and
        `gdm.utils.files.parse_fixed_width_filenames` for more information.
    ...
    strict_time_flag : bool
        If true, gdm will check data to ensure times are unique and
    ...
    ::
        ...
        vefi = gdm.Instrument(platform='cnofs', name='vefi', tag='dc_b')
        ...
        ivm = gdm.Instrument(platform='cnofs', name='ivm')
        ...
        # COSMIC support code.
        cosmic = gdm.Instrument('cosmic', 'gps', 'ionprf', altitude_bin=3)
    ...
    def __init__(self, platform=None, name=None, tag='', inst_id='',
                 ...
                 inst_module=None, data_dir='', directory_format=None,
                 file_format=None, temporary_file_list=False,
                 strict_time_flag=True, ignore_empty_files=False, custom=None,
                 **kwargs):
        """Initialize `gdm.Instrument` object."""
        ...
        self.clean_level = (clean_level.lower() if clean_level is not None
                            else gdm.params['clean_level'])

        # Assign `strict_time_flag`
        self.strict_time_flag = strict_time_flag

        # Assign directory format information, which tells gdm how to look in
        ...
        if self.directory_format is not None:
            ...
            self.directory_format = gdm.params['directory_format']
        ...
        # standard gdm directory structure
        if os.path.isdir(data_dir):
            self.data_dir = data_dir
        else:
            if len(data_dir) > 0:
                gdm.logger.warning("data directory doesn't exist: {:}".format(
                    data_dir))
            self.data_dir = None
        ...
        if ignore_empty_files is None:
            ignore_empty_files = gdm.params['ignore_empty_files']
        if update_files is None:
            update_files = gdm.params['update_files']

        self.files = gdm.Files(self, data_dir=self.data_dir,
    ...
    def __str__(self):
        ...
        if not self.empty:
            ...
            output_str += '\nVariable Names:\n'
            output_str += gdm.utils._core.fmt_output_in_cols(self.variables)
        else:
    ...
    def __setitem__(self, key, new_data):
        """Set data in `gdm.Instrument` object.
    ...
    def __delitem__(self, key):
        ...
        key : str or list-like
            A data variable, label, or attribute; which are considered in that
            order.
    ...
    def __iter__(self):
        ...
        ::
            ...
            inst = gdm.Instrument(platform=platform, name=name, tag=tag)
            start = dt.datetime(2009, 1, 1)
        ...
        # Add last loaded data from local_inst into the original object. Make a
        # copy here to ensure there are no left over references to the
        # `local_inst` object in the loop that would interfere with garbage
        # collection. We don't want to make a copy of underlying data.
    ...
    def _assign_attrs(self, by_name=False):
        ...
        # if none is supplied
        if by_name:
            # gdm platform is reserved for modules within `gdm.instruments`
            if self.platform == 'gdm':
                # Look within gdm
                ...
                user_modules = gdm.params['user_modules']
        ...
        if len(missing) > 0:
            gdm.logger.debug(' '.join(['These Instrument attributes kept',
        ...
        if len(missing) > 0:
            gdm.logger.debug(' '.join(['These Instrument test attributes',
    ...
    def _load_data(self, date=None, fid=None, inc=None, load_kwargs=None):
        ...
        date = gdm.utils.time.filter_datetime_input(date)
        ...
        if len(fname) > 0:
            ...
            try:
                data = self._load_rtn(load_fname, tag=self.tag,
                                      inst_id=self.inst_id, **load_kwargs)
                bad_datetime = False
    ...
    def _set_load_parameters(self, date=None, fid=None):
        ...
        if date is not None:
            year, doy = gdm.utils.time.getyrdoy(date)
            self.yr = year
            self.doy = doy
    ...
    def bounds(self):
        ...
        ::
            ...
            inst = gdm.Instrument(platform=platform,
                                    name=name,
                                    tag=tag)
            start = dt.datetime(2009, 1, 1)
            stop = dt.datetime(2009, 1, 31)
    ...
    def bounds(self, value=None):
        ...
        if hasattr(self.files.files.index, "freqstr"):
            ...
            if file_freq is None:
                if len(self.files.files.index) > 1:
                    ...
                    file_freq = gdm.utils.time.calc_freq(
                        self.files.files.index)
                else:
                    file_freq = '1D'  # This is the gdm default
        else:
            file_freq = '1D'  # This is the gdm default
        ...
        if (start is None) and (stop is None):
            ...
            if self._iter_start[0] is not None:
                ...
                self._iter_list = gdm.utils.time.create_date_range(
            ...
            starts = gdm.utils.listify(start)
            stops = gdm.utils.listify(stop)
            ...
            if isinstance(starts[0], str) or isinstance(stops[0], str):
                ...
                if len(good_bounds) > 0:
                    ...
                    starts = list(gdm.utils.time.filter_datetime_input(
                        np.array(starts)[good_bounds]))
                    stops = list(gdm.utils.time.filter_datetime_input(
    ...
    def date(self, new_date):
        ...
        self._date = gdm.utils.time.filter_datetime_input(new_date)
    ...
    def copy(self):
        ...
        # The files class copy() not invoked with deepcopy
    ...
    def custom_attach(self, function, at_pos='end', args=None, kwargs=None):
        ...
        if at_pos not in pos_list:
            gdm.logger.warning(''.join(['unknown position specified, including',
    ...
    def rename(self, mapper, lowercase_data_labels=False):
        """Rename variables within the data.
        ...
        gdm supports differing case for variable labels across the data object
        ...
        ::
            ...
            # Note that 'Gdm_UTS' was applied to data as 'gdm_uts'
            print(inst['gdm_uts'])
    ...
    def load(self, yr=None, doy=None, end_yr=None, end_doy=None, date=None,
        ...
        ----------
        yr : int or NoneType
            Year for desired data. gdm will load all files with an
        ...
        end_doy : int or NoneType
            ...
            `end_yr`, `end_doy`. (default=None)
        date : dt.datetime or NoneType
            Date to load data. gdm will load all files with an associated
        ...
        **kwargs : dict
        ...
        if len(self.files.files) == 0:
            estr = ''.join(('No files found for Instrument. If files are ',
                            ...
                            "on the system and that gdm.params['data_dirs']",
                            " is set correctly."))
        ...
        if (self.pad is not None) or self.multi_file_day:
            if self._empty(self._next_data) and self._empty(self._prev_data):
                ...
                gdm.logger.debug('Initializing data cache.')

                # Using current date or fid
                self._prev_data = self._load_prev(load_kwargs=kwargs)
                self._curr_data = self._load_data(
                ...
                if self._next_data_track == curr:
                    gdm.logger.debug('Using data cache. Loading next.')
                    ...
                    gdm.logger.debug('Using data cache. Loading previous.')
                    ...
                    # access
                    gdm.logger.debug('Resetting data cache.')
                    del self._prev_data
                    del self._curr_data
                    del self._next_data
                    self._prev_data = self._load_prev(load_kwargs=kwargs)
            ...
            self.data = self._load_data(date=self.date, fid=self._fid,
        ...
        if not self._load_by_date and not self.empty:
            ...
            self.yr, self.doy = gdm.utils.time.getyrdoy(self.date)
    ...
    def download_updated_files(self, **kwargs):
        ...
        **kwargs : dict
        ...
        if remote_files is not None and remote_files.empty:
            gdm.logger.warning(' '.join(('No remote files found. Unable to',
        ...
        if len(new_dates) > 0:
            ...
            gdm.logger.info('Did not find any new or updated files.')
    ...
    def download(self, start=None, stop=None, date_array=None,
        ...
        **kwargs : dict
        ...
        if date_array is None:
            ...
            # whole days.
            start = gdm.utils.time.filter_datetime_input(start)
            stop = gdm.utils.time.filter_datetime_input(stop)
        ...
        if len(date_array) > 0:
            ...
            gdm.logger.info('Updating gdm file list')
            self.files.refresh()
            ...
            if len(self.bounds[0]) == 1:
                ...
                if self._iter_type == 'date':
                    if all([curr_bound[0][0] == first_date,
                        ...
                        gdm.logger.info(' '.join(('Updating instrument',
            ...
            gdm.logger.warning('Requested download over an empty date range.')
    ...
    def to_netcdf4(self, fname, base_instrument=None, epoch_name=None,
        ...
        export_gdm_info : bool
            ...
            operational uses (e.g., conversion of Level 1 to Level 2 data), it
            may be desirable to set this to false to avoid conflicting versions
            of these parameters. (default=True)
        unlimited_time : bool
...
# Hidden variable to store gdm reserved keywords. Defined here, since these
...
def _get_supported_keywords(local_func):
    ...
    local_func : function or method
        Method used to load data within gdm
