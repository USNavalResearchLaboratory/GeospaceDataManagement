...
import GeospaceDataManagement as gdm  # Needed to access gdm.params
...
class Files(object):
    ...
    Parameters
    ----------
    inst : GeospaceDataManagement.Instrument
        Instrument object
    data_dir : str or NoneType
        Directory without sub-directory variables that allows one to bypass the
        directories provided by gdm.params['data_dirs'].  Only applied if the
        directory exists. (default=None)
    directory_format : str or NoneType
        Sub-directory naming structure, which is expected to exist or be
        created within one of the `gdm.params['data_dirs']` directories.
    ...
    ignore_empty_files
    home_path : str
        Path to the GeospaceDataManagement information directory.
    data_path : str
        Path to the top-level directory containing instrument files,
        selected from data_paths.
    data_paths: list of str
        Available paths that GeospaceDataManagement will use when looking for
        files. The class uses the first directory with relevant data, stored in
        `data_path`.
    files : pds.Series
        Series of data files, indexed by file start time.
    inst_info : dict
        Contains GeospaceDataManagement.Instrument parameters 'platform',
        'name', 'tag', and 'inst_id', identifying the source of the files.
    ...
    list_files_rtn : method
        ...
        by associated GeospaceDataManagement.Instrument object.
    multi_file_day : bool
        Flag copied from associated GeospaceDataManagement.Instrument object
        that indicates when data for day n may be found in files for days n-1,
        or n+1
    ...
    class mediates access to the files by datetime and contains helper methods
    for determining the presence of new files and filtering out empty files.

    User should generally use the interface provided by a `gdm.Instrument`
    ...
    ::
        ...
        # Search GeospaceDataManagement appropriate directory for instrument
        # files and update Files instance.
        inst.files.refresh()
    ...
    def __init__(self, inst, data_dir=None, directory_format=None,
        ...
        """Initialize `GeospaceDataManagement.Files` object."""

        # Set the hidden variables
        self.update_files = update_files

        # Location of directory to store file information in
        self.home_path = os.path.join(gdm.gdm_dir, 'instruments')
        ...
        # Set the path for sub-directories under gdm data path
        if directory_format is None:
            # Assign stored template if user doesn't provide one.
            directory_format = gdm.params['directory_format']
        ...
        # Ensure we have at least one path for gdm data directory
        if len(gdm.params['data_dirs']) == 0:
            raise NameError("".join(("GeospaceDataManagement's `data_dirs` ",
                                     "hasn't been set. Please set a top-level ",
                                     "directory path to store data using",
                                     " `gdm.params['data_dirs'] = path`")))

        # Get list of potential data directory paths from gdm. Construct
        # possible locations for data.
        self.data_paths = [os.path.join(pdir, self.sub_dir_path)
                           for pdir in gdm.params['data_dirs']]
        ...
        # list of found files in ~/.gdm. This stored info is retrieved by
        ...
        if self.inst_info['platform'] != '':
            ...
            # `gdm.Instrument` instance, not `gdm.Instrument()`
    ...
    def __repr__(self):
        ...
        out_str = "".join(["GeospaceDataManagement.Files(", inst_repr,
                           ", directory_format=",
    ...
    def _ensure_unique_file_datetimes(self):
        ...
        if not self.multi_file_day and not unique_files:
            ...
            gdm.logger.warning(estr)
            ind = self.files.index.duplicated()
            gdm.logger.warning(self.files.index[ind].unique())
    ...
    def _store(self):
        ...
        Moves current filelist, if different, to .gdm/archive
    ...
    def refresh(self):
        ...
        by `GeospaceDataManagement.params['data_dirs'] = path`.

        """

        # Let interested users know GeospaceDataManagement is searching for
        info_str = '{platform} {name} {tag} {inst_id}'.format(
            **self.inst_info)
        info_str = " ".join(("GeospaceDataManagement is searching for",
                             info_str, "files."))
        info_str = " ".join(info_str.split())  # Remove duplicate whitespace
        gdm.logger.info(info_str)
        ...
        for path in self.data_paths:
            ...
            # focus and features of the original GeospaceDataManagement intact.
        ...
        gdm.logger.info('Found {:d} local files.'.format(len(new_files)))

        if not new_files.empty:
            # Sort files to ensure they are in order
            new_files = new_files.sort_index()
        elif gdm.params['warn_empty_file_list']:
            # Warn user if no files found, if `gdm.params` set
    ...
    def set_top_level_directory(self, path):
        ...
        path and the associated GeospaceDataManagement subdirectories derived
        from the `directory_format` attribute as stored in `self.sub_dir_path`

        Parameters
        ----------
        path : str
            Top-level path to use when looking for files. Must be in
            `GeospaceDataManagement.params['data_dirs']`.

        Raises
        ------
        ValueError
            If `path` not in `GeospaceDataManagement.params['data_dirs']`
        ...
        if path not in gdm.params['data_dirs']:
            estr = "".join(["Supplied path not in ",
                            "`GeospaceDataManagement.params['data_dirs']`"])
    ...
    def from_os(cls, data_path=None, format_str=None,
        ...
        data_path : str or NoneType
            ...
            is provided by GeospaceDataManagement to the
            `instrument_module.list_files`
        ...
        pds.Series
            ...
            `GeospaceDataManagement.utils.files.process_parsed_filenames` for
            details.
