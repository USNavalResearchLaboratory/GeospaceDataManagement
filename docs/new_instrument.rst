...
:py:mod:`GeospaceDataManagement` works by calling modules written for specific
instruments that load and process the data consistent with the
:py:mod:`GeospaceDataManagement` standard. The name of the module corresponds to
the combination 'platform_name' provided when initializing a
:py:mod:`GeospaceDataManagement` instrument object. The module should be placed
in the :py:mod:`GeospaceDataManagement` instruments directory or registered
(see below) for automatic discovery. A compatible module may also be supplied
directly using

.. code:: Python

    GeospaceDataManagement.Instrument(inst_module=python_module_object)


A general template has also been included to make starting any Instrument
module easier at
:py:mod:`GeospaceDataManagement.instruments.templates.template_instrument`.

External modules may be registered as part of the
:py:mod:`GeospaceDataManagement` user instrument registry using the following
syntax:

.. code-block:: python

  from GeospaceDataManagement.utils import registry
...
.. code-block:: python
  ...
  inst = GeospaceDataManagement.Instrument('myplatform', 'myname')
...
--------------------
:py:mod:`GeospaceDataManagement` instruments can reside in external libraries.
The registry methods described above can be used to provide links to these
instrument libraries for rapid access.
...
:py:mod:`GeospaceDataManagement` uses a hierarchy of named variables to define
each specific data product. In order this is:
...
example of this are many of the space weather indices, which may be obtained
from different providers. In this case, the solar and geomagnetic indices may
be included in a common platform (e.g., 'sw' for Space Weather), regardless of
their origin. This allows users to access a given index using different
:py:attr:`inst_id` and :py:attr:`tag` values, even if the mission or observatory
that produced the indices differ.
...
different profile products for COSMIC GPS data, 'ionprf', 'atmprf', ...).
...
TIMED GUVI data for the 'sdr-imaging' and 'sdr-spectrograph' :py:attr:`tag`
values. As a rule, when trying to decide if a characteristic should be assigned
as a :py:attr:`tag` or :py:attr:`inst_id` attribute, the :py:attr:`inst_id`
value should subdivide the :py:attr:`tag` data set in a clear way that does not
...
The DMSP IVM (dmsp_ivm) instrument  is a practical example of a
:py:mod:`GeospaceDataManagement` instrument that uses all levels of variable
names. An :ref:`api-instrument-template` is also provided within
:py:mod:`GeospaceDataManagement`.

Note that during instantiation of a
:py:class:`GeospaceDataManagement.Instrument`,
:py:mod:`GeospaceDataManagement` uses the :py:attr:`tags` and
:py:attr:`inst_ids` above to determine if the values provided by a user are
...
:py:mod:`GeospaceDataManagement` also requires that instruments include
information pertaining to acknowledgements and references for an instrument.
These are simply defined as strings at the instrument level.  In the most basic
case, these can be defined with the data information at the top.

:py:mod:`GeospaceDataManagement` also requires that a logger handle be defined
and instrument information pertaining to acknowledgements and references be
included.  These ensure that people using the data know who to contact with
questions and what they should reference when publishing their results.  The
logging handle should be assigned to the :py:mod:`GeospaceDataManagement`
logger handle, while the references and acknowledgments are defined as
instrument attributes within the initialization method.
...
.. code:: python
  ...
  def init(self):
      ...
      GeospaceDataManagement.logger.info(self.acknowledgements)
      return
...
:py:mod:`GeospaceDataManagement` operations, with functionality to cover finding
files, loading data from specified files, and downloading new files. While the
methods below are sufficient to engage with :py:mod:`GeospaceDataManagement`,
additional optional methods are needed for full :py:mod:`GeospaceDataManagement`
support.

Note that these methods are not directly invoked by the user, but by
:py:mod:`GeospaceDataManagement` as needed in response to user inputs.
...
:py:mod:`GeospaceDataManagement` instruments.
...
``self`` is a  :py:class:`GeospaceDataManagement.Instrument` object.
:py:func:`init` should modify ``self`` in-place as needed; equivalent to a
custom routine.  It is expected to attach the :py:attr:`acknowledgements` and
:py:attr:`references` attributes to ``self``.


list_files
^^^^^^^^^^

:py:mod:`GeospaceDataManagement` maintains a list of files to enable data
management functionality. To get this information
:py:mod:`GeospaceDataManagement` expects a module function
...
:py:attr:`inst_id` and :py:attr:`tag` are passed in by
:py:mod:`GeospaceDataManagement` to select a specific subset of the available
data. The location on the local filesystem to search for the files is passed in
``data_path``. The :py:meth:`list_files` method must return a
:py:class:`pandas.Series` of filenames indexed by datetime objects.

A user must also supply a file template string suitable for locating files
on their system at GeospaceDataManagement.Instrument instantiation, passed via
``format_str``, that must be supported. Sometimes users obtain files from
non-traditional sources and ``format_str`` makes it easier for those users to
use an existing instrument module to work with those files.

:py:mod:`GeospaceDataManagement` will by default store data in
``gdm_data_dir/platform/name/tag/inst_id``, helpfully provided in
``data_path``, where gdm_data_dir is specified by using
``GeospaceDataManagement.params['data_dirs'] = gdm_data_dir``. Note that an
alternative directory structure may be specified using the
:py:class:`GeospaceDataManagement.Instrument` keyword ``directory_format`` at
instantiation. The default is recreated using

.. code:: python

    dformat = '{platform}/{name}/{tag}/{inst_id}'
    inst=GeospaceDataManagement.Instrument(platform, name,
                                           directory_format=dformat)

Note that :py:mod:`GeospaceDataManagement` handles the path information thus
instrument module developers do not need to do anything to support the
``directory_format`` keyword.

Pre-Built list_files Methods and Support
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Finding local files is generally similar across data sets thus
:py:mod:`GeospaceDataManagement` includes a variety of methods to make
supporting this functionality easier. The simplest way to construct a valid
list_files method is to use one of these included
:py:mod:`GeospaceDataManagement` methods.

A complete method is available in
:py:func:`GeospaceDataManagement.instruments.methods.general.list_files` that
may find broad use.

:py:meth:`GeospaceDataManagement.Files.from_os` is a convenience constructor
provided for filenames that include time information in the filename and
utilize a constant field width or a consistent delimiter. The location and
format of the time information is specified using standard python formatting
and keywords ``year``, ``month``, ``day`` of month (year), ``hour``, ``minute``,
``second``. Additionally, ``version``, ``revision``, and ``cycle`` keywords are
supported. When present, the :py:meth:`GeospaceDataManagement.Files.from_os`
constructor will filter down the file list to the latest
version/revision/cycle combination. Additional user specified template variables
...
.. code:: python
   ...
   def list_files(tag='', inst_id='', data_path='', format_str=None):
       ...
       return GeospaceDataManagement.Files.from_os(data_path=data_path,
...
situations however both remain within :py:mod:`GeospaceDataManagement` to
support currently unknown edge cases that users may encounter. More practically,
parsing with a delimiter offers more support for the ``*`` wildcard than the
fixed width parser. It is generally advised to limit use of the ``*`` wildcard
to prevent potential false positives if a directory has more than one instrument
within.

If the constructor is not appropriate, then lower level methods within
:py:class:`GeospaceDataManagement.Files` may also be used to reduce the workload
in adding a new instrument. Access to the values of user provided template
variables is not available via :py:meth:`GeospaceDataManagement.Files.from_os`
and thus requires use of the same lower level methods in
:py:mod:`GeospaceDataManagement.utils.files`.

See :py:func:`GeospaceDataManagement.utils.time.create_datetime_index` for
creating a datetime index for an array of irregularly sampled times.

:py:mod:`GeospaceDataManagement` will invoke the list_files method the first
time a particular instrument is instantiated. After the first instantiation, by
default :ref:`tutorial-params`, :py:mod:`GeospaceDataManagement` will not search
for instrument files as some missions can produce a large number of files, which
may take time to identify. The list of files associated with an Instrument may
be updated by adding ``update_files=True`` to the kwargs.

.. code:: python

   inst = GeospaceDataManagement.Instrument(platform=platform, name=name,
                                            update_files=True)
...
:py:mod:`GeospaceDataManagement` instruments. The work invested by the
instrument module author makes it possible for users to work with the data
easily.
...
- :py:data:`fnames` contains a list of filenames with the complete data path
  ...
  With most data sets the method should return the exact data that is within
  the file. However, :py:mod:`GeospaceDataManagement` is also currently
  optimized for working with data by day. This can present some issues for data
  sets that are stored by month or by year. See :ref:`instruments-sw` for
  examples of data sets stored by month(s).
...
- If your data is a CSV formatted file, you can incorporate the
  :py:func:`GeospaceDataManagement.instruments.methods.general.load_csv_data`
  routine (see :ref:`api-methods-general`) into your :py:func:`load` method.
...
provided by 'data_path', which will be supplied by
:py:mod:`GeospaceDataManagement`.
...
for this Instrument will be stored within the :py:mod:`GeospaceDataManagement`
data directory. If not set or if set to ``None``, it defaults to
...
If provided, :py:mod:`GeospaceDataManagement` supports the definition and use of
keywords for an instrument module so that users may define their preferred
default values. A custom keyword for an instrument module must be defined in
each function that will receive that keyword argument if provided by the user.
All instrument functions, :py:func:`init`, :py:func:`preprocess`,
:py:func:`concat_data`, :py:func:`load`, :py:func:`clean`,
:py:func:`list_files`, :py:func:`list_remote_files`, and :py:func:`download`
support custom keywords. The same keyword may be used in more than one function
but the same value will be passed to each.
...
then :py:mod:`GeospaceDataManagement` will pass those custom keyword arguments
to :py:func:`load` with every call. All user provided custom keywords are copied
into the :py:class:`~GeospaceDataManagement.Instrument` object itself under
:py:attr:`inst.kwargs` for use in other areas. All available keywords, including
default values, are also grouped by relevant function in a dictionary,
:py:attr:`inst.kwargs_supported`, attached to the :py:class:`Instrument` object.
Updates to values in :py:attr:`inst.kwargs` will be propagated to the relevant
function the next time that function is invoked.

.. code:: python

   inst = GeospaceDataManagement.Instrument(platform, name, custom1=new_value)
   ...
   # Show keywords reserved for use by GeospaceDataManagement
   print(inst.kwargs_reserved)

If a user supplies a keyword that is reserved or not supported by
:py:mod:`GeospaceDataManagement`, or by any specific instrument module function,
then an error is raised. Reserved keywords are :py:data:`fnames`,
:py:data:`inst_id`, :py:data:`tag`, :py:data:`date_array`, :py:data:`data_path`,
...
``self`` is a :py:class:`GeospaceDataManagement.Instrument` object.
:py:func:`preprocess` should modify ``self`` in-place as needed; equivalent to
a custom routine.
...
``self`` is a :py:class:`GeospaceDataManagement.Instrument` object.
:py:func:`clean` should modify ``self`` in-place as needed; equivalent to a
custom routine. :py:func:`clean` is allowed to raise logger messages, warnings,
and errors. If the routine does this, be sure to test them by assigning the
necessary information to the :py:attr:`_clean_warn` attribute, described in
:ref:`rst_test-clean`. :py:func:`clean` may also re-assign the cleaning level if
...
to date data. :py:mod:`GeospaceDataManagement` developers highly encourage the
development of this method, when possible.
...
This method is called by several internal :py:mod:`GeospaceDataManagement`
functions, and can be directly called by the user through the
:py:meth:`inst.remote_file_list` method. The user can search for subsets of
files through optional keywords, such as:
...
:py:meth:`~GeospaceDataManagement._instrument.Instrument.concat_data`, after
the default method handles the prepending of the data that needs to be combined.
...
:py:mod:`GeospaceDataManagement` is connected to the Python logging module. This
allows users to set the desired level of direct feedback, as well as where
feedback statements are delivered. The following line in each module is
encouraged at the top-level so that the instrument module can provide feedback
using the same mechanism

.. code:: Python

    logger = GeospaceDataManagement.logger
...
.. code:: Python
    ...
    GeospaceDataManagement.logger.info(information_string)
    GeospaceDataManagement.logger.warning(warning_string)
    GeospaceDataManagement.logger.debug(debug_string)
...
Testing Support
===============
All modules defined in the ``__init__.py`` for
``GeospaceDataManagement/instruments`` are automatically tested when
:py:mod:`GeospaceDataManagement` code is tested. To support testing all
...
The standardized :py:mod:`GeospaceDataManagement` tests are available in
:py:mod:`GeospaceDataManagement.tests.instrument_test_class`. The test
collection in test_instruments.py imports this class, collects a list of all
available instruments (including potential :py:attr:`tag`/:py:attr:`inst_id`
combinations), and runs the tests using pytestmark.  By default,
:py:mod:`GeospaceDataManagement` assumes that your instrument has a fully
functional download routine, and will run an end-to-end test.  If this is not
the case, see the next section.
...
:py:class:`~GeospaceDataManagement._instrument.Instrument`. The descriptions
should provide insight into when and how they should be used.
...
For instance, say we have an instrument team that wants to use
:py:mod:`GeospaceDataManagement` to manage their data products.  Level 1 data is
locally generated by the team, and Level 2 data is provided to a public
repository.  The instrument should be set up as follows:
...
Instrument templates may be found at
:py:mod:`GeospaceDataManagement.instruments.templates` and supporting methods
may be found at :py:mod:`GeospaceDataManagement.instruments.methods`.

General
^^^^^^^

A general instrument template is included with :py:mod:`GeospaceDataManagement`,
:py:mod:`GeospaceDataManagement.instruments.templates.template_instrument`, that
has the full set of required and optional methods, and docstrings, that may be
used as a starting point for adding a new instrument to
:py:mod:`GeospaceDataManagement`.
...
Note that :py:mod:`GeospaceDataManagement` assumes that this flag is True if no
variable is present. Thus, specifying only
``_test_download = {'': {'Level_1': False}}`` has the same effect, and Level 2
...
Sometimes standard tests ensure that GeospaceDataManagement that ensure
all data handling features work as expected throughout may data types. For
example, GeospaceDataManagement has tests for loading multiple days at a time
or using a data pad. When these tests require significant updates, an additional
flag may be used to suppress these tests temporarily for specific instruments
while updates are made to that instrument. These new tests are run by default
unless specified using the :py:attr:`_new_tests` flag.
