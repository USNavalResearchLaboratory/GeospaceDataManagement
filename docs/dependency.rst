...
Using GeospaceDataManagement as a dependency
============================================
...
:py:mod:`pytest` and the test_instruments script.  GeospaceDataManagement
includes a standard suite of instrument tests to run on instruments.  These are
imported from the :py:mod:`GeospaceDataManagement.tests.instrument_test_class`
in the main GeospaceDataManagement test library.  The ``test_instruments.py``
file can be copied directly into the library, updating the instrument library
name as indicated.

The ``pyproject.toml`` file should include GeospaceDataManagement as a
dependency, as well as any other packages required by the instruments.

A more complicated structure could include analysis routines  or methods for
common analysis routines used for a specific data set. The structure then could
look like:
...
.. _GeospaceDataManagement-dep-testinst:

Using GeospaceDataManagement to test your instruments
-----------------------------------------------------

A generalized instrument test class is provided under
:py:mod:`GeospaceDataManagement.tests` for developers.  Continuing the above
example, developers may copy over the ``test_instruments.py`` file and update it
in a few locations.  For example:
...
.. code:: python
  ...
  # Import the test classes from GeospaceDataManagement
  from GeospaceDataManagement.tests.classes.cls_instrument_library import InstLibTests
...
.. code:: python
 ...
 user_info = {'gdm_testing': {'user': 'gdm_testing', 'password': 'your@email'}}
 InstLibTests.initialize_test_package(InstLibTests,
                                      inst_loc=customLibrary.instruments,
                                      user_info=user_info)
...
.. code:: Python
  ...
  GeospaceDataManagement instrument test class.
...
Adding custom tests in GeospaceDataManagement
---------------------------------------------
...
functions?  GeospaceDataManagement includes a series of test instrument objects
that can be imported by other packages to test those functions.  The test
instruments are used as part of the unit tests.  This allows us to thoroughly
test routines without including a large volume of data as part of the package.

gdm_testing
^^^^^^^^^^^
:ref:`api-gdm-testing` is the basic test object.  It returns a satellite-like
object with 1D data as a function of latitude, longitude, and altitude in a
pandas format.  Most similar to in situ data.

gdm_ndtesting
^^^^^^^^^^^^^
:ref:`api-gdm-ndtesting` is a satellite-like object that returns all
of the above plus an imager-like data set, ie, remote data that is a function
of time and two spatial dimensions.

gdm_testmodel
^^^^^^^^^^^^^
:ref:`api-gdm-testmodel` is an xarray object that returns a 4D object as a
...
generate dates between 1 Jan 2008 and 31 Dec 2010 for use within
GeospaceDataManagement.  This allows for coverage for year changes both with
and without leap days.
...
Remember to include GeospaceDataManagement as a dependency in your
pyproject.toml file.

The CI environment will also need to be configured to install
GeospaceDataManagement and its dependencies.  You may need to install
GeospaceDataManagement from GitHub rather than pip if you need to test against
a specific development branch.

If the GeospaceDataManagement API is changing for an upcoming release, you can
use :py:mod:`packaging` to quickly determine the GeospaceDataManagement version
and potentially skip tests that are only necessary for a limited range of
GeospaceDataManagement versions.
...
.. code:: python
  ...
  @pytest.mark.skipif(Version(gdm.__version__) == Version('0.0.1'),
                      reason=''.join(('Requires test model in ',
		                       'GeospaceDataManagement v0.0.1.')))
  def test_new_feature(self):
     """Check a new feature that requires the develop GeospaceDataManagement."""
